...
! ================================================================
real(iwp),parameter::ec=8.0e5;
real(iwp),parameter::pr_sat=0.26;
real(iwp),parameter::eps_s_r=1e-3;
real(iwp),parameter::c=10;
real(iwp),parameter::eta=1.0e-2;
real(iwp),parameter::m=2;
real(iwp),parameter::h0=1.0 ! 100.0;
...
u_n_pol=[electric_polarization_p,electric_field_t]

!write(22,*)'gauss_point_number=',gauss_point_number
!write(22,*)'electric_field_t=',electric_field_t
!write(22,*)'time=',time(2)

!u_n_pol=0.0d0

do polarization_iteration_number=1,max_iteration

!write(22,951)polarization_iteration_number,time(2),u_n_pol


r_n_pol(1:3)=u_n_pol(1:3) &
-( dtime/eta)*( ( macaulay_brackets ( norm_vect(electric_field_t-u_n_pol(4:6) )/ec -1 ) ) ** m ) * &
                          unit_vect( electric_field_t-u_n_pol(4:6) )- electric_polarization_p
r_n_pol(4:6)=u_n_pol(4:6) &
- h0*atanh( norm_vect( u_n_pol(1:3) ) /pr_sat )*unit_vect( u_n_pol(1:3) )
polarization_error=norm_vect(r_n_pol)

ktan_pol=-identity_matrix(6)

ktan_pol(1:3,4:6)=  &
-(m*dtime/eta)*( ( macaulay_brackets ( norm_vect(electric_field_t-u_n_pol(4:6) )/ec -1 ) ) ** (m-1) ) &
*dyad_of_two_vector( unit_vect( electric_field_t-u_n_pol(4:6) ), &
                     unit_vect( electric_field_t-u_n_pol(4:6) ) ) &
-( dtime/eta)*( ( macaulay_brackets ( norm_vect(electric_field_t-u_n_pol(4:6) )/ec -1 ) ) ** m )  &
* derivative_of_direction(electric_field_t-u_n_pol(4:6) )

ktan_pol(4:6,1:3)=-h0*( pr_sat /(norm_vect( u_n_pol(1:3) )**2.0-pr_sat*pr_sat ) ) &
*dyad_of_two_vector( unit_vect( u_n_pol(1:3) ), &
                     unit_vect( u_n_pol(1:3) ) ) &
-h0*atanh( norm_vect( u_n_pol(1:3) ) /pr_sat ) &
*derivative_of_direction(u_n_pol(1:3))
...
write(22,951)polarization_iteration_number,time(2),u_n_pol
         ...
         if (polarization_error.le.tolerance*pr_sat)then
             converged=.true.
             exit
         endif
...
curn_polarization_function(gauss_point_number,:)=u_n_pol(1:3)
